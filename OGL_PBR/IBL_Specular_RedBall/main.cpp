...
// ------------------------------------------------------------------------------
    ...
    float *data = stbi_loadf("/书/OGL_Test/Arches_E_PineTree/Arches_E_PineTree_3k.hdr", &width, &height, &nrComponents, 0);
//    float *data = stbi_loadf("/书/OGL_Test/snow_machine/test8_Ref.hdr", &width, &height, &nrComponents, 0);
    ...
    // pbr IBL2 (specular): create a pre-filter cubemap, and re-scale capture FBO to pre-filter scale.
    // --------------------------------------------------------------------------------
    ...
    // pbr IBL2 (specular): run a quasi monte-carlo simulation on the environment lighting to create a prefilter (cube)map.
    // ----------------------------------------------------------------------------------------------------
    prefilterShader.use();
    ...
    // pbr IBL2 (specular): generate a 2D LUT from the BRDF equations used.
    // ----------------------------------------------------
...
// ======================================================================================================================================================================================
//    //暴力计算环境镜面反射，帧缓冲。（1）（0在pbr_redBall_irradianceMap.fs）
...
//    glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f); //投影矩阵（FOV视锥体竖直张开角度，横向竖直张开角度比，近裁剪面深度，远裁剪面深度）
...
//
//
//        pbrShader.setFloat("metallic", 0.5);//行数从下往上，金属度从低到高
//
//            // 我们将粗糙度限制在0.025-1.0，因为完全光滑的表面（0.0的粗糙度）看起来有点偏离
//            // 在直接照明下 on direct lighting.
//            pbrShader.setFloat("roughness", 0.1);//列数从左到右，粗糙度从小到大
//
//            model = glm::mat4(1.0f);
