...
unsigned int loadTexture(const char *path, bool isRepeat);
...
//    Model ourModel("/书/OGL_Test/nanosuit/nanosuit.obj");
    ...
    Shader screenShader("/书/OGL_Test/screen.vs", "/书/OGL_Test/screen.fs");
    screenShader.use();
...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);//帧缓冲：一张纹理六个顶点
    ...
    while (!glfwWindowShouldClose(window)) //每次循环的开始前检查一次GLFW是否被要求退出
    {
        ...
        // 在渲染窗户之前进行排序 sort the transparent windows before rendering
